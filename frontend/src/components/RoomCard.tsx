...
// Hoist static SVG icons to avoid recreating on every render
const ROOM_ICON = (
...
// Hoist placeholder avatars
const PLACEHOLDER_AVATARS = [1, 2, 3].map((i) => (
  <div key={i} className="w-4 h-4 bg-midnight-800 border border-white/10 skew-x-[-6deg]" />
));

function RoomCard({ room, variant = 'default', showActions = false, onDelete, isDeleting = false }: RoomCardProps) {
  ...
  const isPublic = room.isPublic || (room as any).public;
  ...
  return (
    <Card hover className="p-8 border-white/5 bg-midnight-900/40">
      ...
      <div className="flex gap-4 room-card-actions">
