    ...
    return (
      <div className="min-h-screen bg-surface-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-surface-500 mb-6">This room may have been deleted or made private.</p>
          <Link
            to="/explore"
    ...
    <div className="min-h-screen bg-surface-50 text-surface-900 font-sans selection:bg-primary-500 selection:text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-2">{room.name}</h1>
          <p className="text-surface-500">Tracking {room.addresses?.length || 0} {(room.addresses?.length || 0) === 1 ? 'address' : 'addresses'}</p>
        </div>
            ...
            <div className="bg-white rounded-3xl border border-surface-200 p-6 shadow-card">
              ...
              <div className="space-y-2">
                {room.addresses?.map((addr) => (
                  <div key={addr.id} className="flex items-center gap-3 p-3 bg-surface-50 rounded-xl">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-surface-200 to-surface-300 flex items-center justify-center flex-shrink-0">
                      <svg className="w-4 h-4 text-surface-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
                    </div>
                    <span className="text-sm font-mono text-surface-700 truncate">{formatAddress(addr.address)}</span>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-3xl border border-surface-200 shadow-card overflow-hidden">
              ...
              <div className="flex border-b border-surface-200">
                ...
                >
                  Positions
                  {activeTab === 'positions' && (
                    <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-surface-900" />
                  )}
                </button>
                <button
                  ...
                  <>
                    {positions.length === 0 ? (
                      <div className="py-16 text-center">
                        ...
                        <p className="text-sm text-surface-400 mt-1">Positions will appear when tracked addresses hold shares</p>
                      </div>
                    ) : (
                            ...
                            >
                              <div className="flex items-start justify-between gap-4">
                                ...
                                <div className="text-right">
                                  <div className={`text-lg font-bold ${position.cashPnl >= 0 ? 'text-success-600' : 'text-red-600'}`}>
                                    {position.cashPnl >= 0 ? '+' : ''}{position.percentPnl.toFixed(1)}%
                                  </div>
                                  <div className={`text-sm ${position.cashPnl >= 0 ? 'text-success-600' : 'text-red-600'}`}>
                                    {position.cashPnl >= 0 ? '+' : ''}${Math.abs(position.cashPnl).toLocaleString()}
                                  </div>
                                </div>
                              </div>
                            </div>
                  ...
                  <>
                    {activities.length === 0 ? (
                      <div className="py-16 text-center">
                        ...
                        <p className="text-sm text-surface-400 mt-1">Check back later for trading activity</p>
                      </div>
                    ) : (
                      <div className="space-y-3">
